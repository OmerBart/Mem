    ...
    switch (btn.id) {
        case "large":
            ...
            elAllCards.forEach((card) => {
                ...
                if (possibleCards.includes(dataCard) && dataCard <= 10 ) {
        ...
        case "medium":
            ...
            elAllCards.forEach((card) => {
                ...
                if (possibleCards.includes(dataCard) && dataCard <= 6) {
                    card.style.display = "flex";
                } else {
                    card.style.display = "none";
                }
        ...
        case "small":
            ...
            elAllCards.forEach((card) => {
                ...
                if (possibleCards.includes(dataCard) && dataCard <= 4) {
                    card.style.display = "flex";
                } else {
                    card.style.display = "none";
                }
        ...
        case "countdown":
            ...
            break;
        case "user":
            const elInput = document.querySelector('[name=pairs]')
            const userInput = getLegalPairs(elInput.value);
            gameMode = 5;
            TOTAL_CARD_PAIRS = userInput;

            elGameModeSpan.innerText = `UserInput`;
            elBestScoreSpan.innerText = formatCounterToTime(
                localStorage.getItem("bestScore")
            );
            elWorstScoreSpan.innerText = formatCounterToTime(
                localStorage.getItem("worstScore")
            );
            showCards(1500);
            elAllCards.forEach((card) => {
                let dataCard = card.getAttribute("data-card");
                console.log(getPossibleCards(userInput));
                let possibleCards = getPossibleCards(userInput);
                //let possibleCards = "12345678";
                if (possibleCards.includes(dataCard) && dataCard < userInput ) {
                    card.style.display = "flex";
                } else {
                    card.style.display = "none";
                }
                ...
                card.classList.add("UserInput-mode");
                pickModeContainer.classList.remove("show");
            });
            break;
    }
...
const firstTenPairs = "12345678910";
const lastFivePairs = "1112131415";
//const potentialCars = "123456789101112131415";


function onCustompairs() {
    //ev.preventDefault()
    const elInput = document.querySelector('[name=pairs]')
    const userInput = getLegalPairs(elInput.value);
    gameMode = 5;
...
function getPossibleCards(userInput) {
    if (userInput == 10){
        return firstTenPairs;
    }
    else if (userInput < 5) {
        return firstTenPairs.slice(0, userInput*2 );
    }
    else if (userInput > 5 && userInput < 10) {
        if(userInput == 7){
            return firstTenPairs + lastFivePairs.slice(0, userInput*2-6);
        }
        else if (userInput == 8){
            return firstTenPairs + lastFivePairs.slice(0, userInput*2-4);
        }
        else if (userInput == 9){
            return firstTenPairs + lastFivePairs.slice(0, userInput*2-2);
        }
        return firstTenPairs + lastFivePairs.slice(0, userInput*2-8);
    }
    else {
        return firstTenPairs + lastFivePairs.slice(0, userInput*2-21);
    }
